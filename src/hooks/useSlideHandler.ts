...
export default () => {
  ...
  /**
   * Move slide focus
   * @param command Slide focus movement command: Move Up, Move Down
   */
  ...
  // Encrypt and copy current slide data to clipboard
  const copySlide = () => {
  ...
  // Try to decrypt and paste slide data from clipboard
  const pasteSlide = () => {
  ...
  // Create a blank slide and append it next
  const createSlide = () => {
    const emptySlide: Slide = {
      id: nanoid(10),
      elements: [],
  ...
  // Create a new slide based on template
  ...
  // Delete current slide. If deleting all slides, reset presentation
  ...
  // Cut current slide
  // Cache slide IDs to delete beforehand since copying clears multi-selection state
  const cutSlide = () => {
  ...
  // Sync data after dragging to reorder slides
