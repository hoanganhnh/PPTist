...
export default () => {
  ...
  // Format color to Opacity + HexString for pptxgenjs
  ...
  // Format HTML string for pptxgenjs library
  // Main logic: split HTML into flat style segments, inheriting parent styles and adding linebreaks for blocks
  ...
  // Format SVG path info for pptxgenjs
  ...
  // Check if base64 image URL
  const isBase64Image = (url: string) => {
    const regex = /^data:image\/[^;]+;base64,/
    return url.match(regex) !== null
  }

  // Check if SVG image URL
