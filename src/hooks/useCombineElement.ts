...
export default () => {
  ...
  /**
   * Determine if the currently selected elements can be grouped
   */
  ...
  /**
   * Group currently selected elements: assign the same group ID to them
   */
  const combineElements = () => {
    ...
    // Gather elements to group and assign a unique group ID
    ...
    // Keep group element layers consecutive in the DOM list:
    // First find the highest layer of this group, remove the elements to be grouped from the list,
    // Then insert the collected group elements back into the list at the highest layer position
  ...
  /**
   * Ungroup elements: remove group ID from selected elements
   */
  const uncombineElements = () => {
    ...
    // Re-evaluate active element state after ungrouping
    // Reset active element state to current active item, or null if none
