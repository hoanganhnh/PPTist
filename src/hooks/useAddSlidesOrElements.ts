...
export default () => {
  ...
  /**
   * Add specified element data (group)
  ...
  const addSlidesFromData = (slides: Slide[]) => {
    ...
    const newSlides = slides.map(slide => {
      ...
      for (const element of slide.elements) {
        ...
        // If the element is linked to slide navigation
        if (element.link && element.link.type === 'slide') {

          // If slides to add include linked slide, update link mapping
          if (slideIdMap[element.link.target]) {
            element.link.target = slideIdMap[element.link.target]
          }
          // If slides to add do not include linked slide, clear link mapping
