...
// Rich text interaction event listeners:
// Disable global shortcuts when focused
// Synchronize data on text input
// Synchronize rich text attributes to toolbar on click or keypress
...
const handleFocus = () => {
  // Do not disable global shortcuts if multiple items are selected and Ctrl/Shift is pressed
...
// Execute rich text commands (single or batch)
// Some commands check if selection is empty first; if so, they select all text
  ...
  for (const item of actions) {
    if (item.command === 'fontname' && item.value !== undefined) {
      ...
      if (item.value && !document.fonts.check(`16px ${item.value}`)) {
        message.warning('Font needs to be downloaded before it takes effect, please wait')
      }
    }
...
// Execute format painter command on mouse up
const handleMouseup = () => {
...
// Prosemirror editor initialization and destruction
onMounted(() => {
