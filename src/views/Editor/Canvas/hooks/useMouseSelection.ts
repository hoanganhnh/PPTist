...
export default (elementList: Ref<PPTElement[]>, viewportRef: ShallowRef<HTMLElement | null>) => {
  ...
  const updateMouseSelection = (e: MouseEvent) => {
    ...
    // Confirm start position of box selection and initialize other defaults
    ...
    document.onmousemove = e => {
      ...
      // Calculate selection box drag direction
      // Distinguish by quadrants, e.g., bottom-right is quadrant 4
      let quadrant = 0
      ...
      // Update drag selection box range
      mouseSelection.value = {
        ...mouseSelection.value,
        width: width,
        height: height,
    ...
    document.onmouseup = () => {
      ...
      // Check if canvas elements fall inside selection box; select them if they do
      ...
      for (let i = 0; i < elementList.value.length; i++) {
        ...
        // Bounding box overlap calculations vary by drag selection directions
        let isInclude = false
        ...
        // Ignore locked or hidden elements even if they lie inside selection range
        if (isInclude && !element.lock && !hiddenElementIdList.value.includes(element.id)) inRangeElementList.push(element)
      }

      // If a group member is in selection box, all members must be enclosed to select the group
