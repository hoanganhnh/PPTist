  ...
  <div class="rich-text-base">
    ...
    <ButtonGroup class="row" passive>
      <Popover trigger="click" v-model:value="AIPopoverVisible" style="width: 25%;">
        <template #content>
          <PopoverMenuItem center @click="execAI('Beautify & Rewrite')">Beautify</PopoverMenuItem>
          <PopoverMenuItem center @click="execAI('Expand & Enrich')">Expand</PopoverMenuItem>
          <PopoverMenuItem center @click="execAI('Simplify & Refine')">Simplify</PopoverMenuItem>
        </template>
        <CheckboxButton
          first
          style="width: 100%;"
    ...
    <div class="row">
      <ButtonGroup style="flex: 1;" passive>
        <Button first style="flex: 1;" v-tooltip="'Decrease Paragraph Indent'" @click="emitRichTextCommand('indent', '-1')"><i-icon-park-outline:indent-left /></Button>
        <Popover trigger="click" v-model:value="indentLeftPanelVisible">
          <template #content>
            <PopoverMenuItem center @click="emitRichTextCommand('textIndent', '-1')">Decrease First Line Indent</PopoverMenuItem>
          </template>
          <Button last class="popover-btn"><i-icon-park-outline:down /></Button>
        </Popover>
      </ButtonGroup>
      <div style="width: 10px;"></div>
      <ButtonGroup style="flex: 1;" passive>
        <Button first style="flex: 1;" v-tooltip="'Increase Paragraph Indent'" @click="emitRichTextCommand('indent', '+1')"><i-icon-park-outline:indent-right /></Button>
        <Popover trigger="click" v-model:value="indentRightPanelVisible">
          <template #content>
            <PopoverMenuItem center @click="emitRichTextCommand('textIndent', '+1')">Increase First Line Indent</PopoverMenuItem>
          </template>
          <Button last class="popover-btn"><i-icon-park-outline:down /></Button>
        </Popover>
      </ButtonGroup>
...
const execAI = async (command: string) => {
  ...
  if (!content) return message.error('No text content to execute')
  ...
  if (typeof stream === 'object' && stream.state === -1) {
    return message.error('The API concurrency for this model is too high, please try a different model')
  }
