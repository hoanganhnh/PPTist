<template>
  <MoveablePanel
    ...
    :width="300"
    :height="560"
    :title="`Slide${slideIndex + 1} Annotations`"
    :left="-270"
    :top="90"
    :minWidth="300"
    :minHeight="400"
  ...
  >
    <div class="container">
      <div class="notes" ref="notesRef">
        ...
        <div class="empty" v-if="!notes.length">No annotations on this slide</div>
      </div>
      <div class="send">
        <TextArea
          ref="textAreaRef"
          ...
          :padding="6"
          :placeholder="`Enter annotation (for ${handleElementId ? 'Selected Elements' : 'Current Slide' }）`"
          :rows="2"
          @focus="replyNoteId = ''; activeNoteId = ''"
          @enter.prevent="createNote()"
        />
        <div class="footer">
          <i-icon-park-outline:delete class="btn icon" v-tooltip="'Clear Annotations on This Slide'" style="flex: 1" @click="clear()" />
          <Button type="primary" class="btn" style="flex: 12" @click="createNote()"><i-icon-park-outline:plus /> Add Annotation</Button>
        </div>
      </div>
    </div>
  </MoveablePanel>
